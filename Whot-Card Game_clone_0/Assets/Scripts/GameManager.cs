...
public class GameManager : MonoBehaviourPunCallbacks
{
    ...
    {
        if (handTransform == playerHandTransform) // Local player hand
        {
            ...
            for (int i = 0; i < hand.Count; i++)
            {
                ...
                card.GetComponent<Card>().ShowFront(); // Show front for local player
            }
        }
        else
        {
            ...
            {
                GameObject card = hand[i];
                card.transform.localPosition = new Vector3(0, i * stackOffset, 0); // Stack opponent's cards
                card.GetComponent<Card>().ShowBack(); // Show back for opponents
            }
        }
    }
    ...
    void RemoveCardFromHand(Card card)
    {
        ...
        if (playerHands[PhotonNetwork.LocalPlayer.ActorNumber].Contains(cardObject))
        {
            ...
            // Only destroy the card if it's not the top card
            if (cardObject != topCard)
            {
                Destroy(cardObject);
            }
    ...
    void RPC_PlayCard(string shape, int number, int actorNumber)
    {
        ...
        if (topCard != null)
        {
            topCard.SetActive(false); // Deactivate the old top card to avoid overlap
            topCard.GetComponent<Collider2D>().enabled = false; // Ensure the collider is disabled
        }

        // Find the new card to play from the player's hand based on shape and number
        ...
        if (playedCard != null)
        {
            // Set this card as the new top card and move it to the discard pile
            ...
            topCard = playedCard; // Update the top card reference
